...
fetch('../public/jsonData/treemap.json')
	...
	.then(({ response }) => {
		const compayClass = new CompanyOrgClass({ elementId: 'section-0' });
...
fetch('../public/jsonData/bloodtest.json')
	...
	.then(({ response }) => {
		const blood1Class = new Blood1Class({ elementId: 'section-2-1' });
...
fetch('../public/jsonData/blood2test.json')
	...
	.then(({ response }) => {
		const blood2Class = new Blood2Class({ elementId: 'section-2-2' });
...
fetch('../public/jsonData/sankey.json')
	...
	.then(({ response }) => {
		const sankeyClass = new SankeyClass({ elementId: 'section-3' });
...
fetch('../public/jsonData/operateData.json')
	...
	.then(({ response }) => {
		const operateClass = new OperateClass({ elementId: 'section-4' });
...
fetch('../public/jsonData/patients.json')
	...
	.then(({ response }) => {
		const patientsClass = new PatientsClass({ elementId: 'section-5' });
...
fetch('../public/jsonData/visitors.json')
	...
	.then(({ response }) => {
		const visitorClass = new VisitorClass({ elementId: 'section-6' });
